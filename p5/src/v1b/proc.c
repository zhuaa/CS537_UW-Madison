...
found:
  ...
  /*
  FRAMES[index] =  V2P(p->kstack) >> 12;
  PIDS[index] = p->pid;
  cprintf("********frame: %d, pid: %d\n", FRAMES[index], PIDS[index]);
  index++;
  */
...
userinit(void)
{
  ...
  p = allocproc();
  //cprintf("~~~~~~~~~~~~~~~inside userinit, after allocproc\n"); 
  initproc = p;
...
fork(void)
{
  ...
  //cprintf("~~~~~~~~~~~~~~~inside fork, after allocproc\n");
  ...
  *np->tf = *curproc->tf;
  ...
  release(&ptable.lock);
  //cprintf("~~~~~~~~~~~~~~~inside fork, before return\n");
  return pid;
}
...
int dump_physmem(int *frames, int *pids, int numframes)
{
  ...
  //cprintf("**********inside dump_physmem, pid of frame 57341: %d\n", FRAMES[57341]);
  /*
